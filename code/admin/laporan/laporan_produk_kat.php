...
$querys = "SELECT nama_kategori from kategori where id_kategori = '$id'";
...
<body>
    ...
    <center>
        ...
        <h2>Data Laporan Stok <?= $row['nama_kategori']; ?> <?=$toko['toko']?> </h2>
        <h6><?= strftime('%A %d %B %Y') ?></h6>
        <h6 class="mr-auto">Oleh : <?php echo $tessss['nama']; ?></h6>
        <br>
    </center>
    <table id="basic-datatables" style="width: 90%; margin: 50px; " class="display table table-striped table-hover">
                            <thead>
                                <tr>
                                    <th style="width: 7%">#</th>
                                    <th>ID Produk</th>
